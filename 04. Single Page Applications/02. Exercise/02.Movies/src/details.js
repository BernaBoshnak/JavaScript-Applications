...
async function onDelete(e, id) {
    e.preventDefault();
    const comfirmed = confirm('Are you sure you want to delete this movie?');
    if (comfirmed) {
        const response = await fetch('http://localhost:3030/data/movies/' + id, {
            method: 'delete',
            headers: {
...
function createMovieCard(movie, likes, ownLike) {
    const conrtrols = create('div', { className: 'col-md-4 text-center' },
    ...
    if (userId !== null) {
        if (userId === movie._ownerId) {
            conrtrols.appendChild(create('a', { className: 'btn btn-danger', href: '#', onClick: (e) => onDelete(e, movie._id) }, 'Delete'));
            conrtrols.appendChild(create('a', { className: 'btn btn-warning', href: '#', onClick: () => showEdit(movie._id) }, 'Edit'));
        } else if (ownLike.length === 0) {
            conrtrols.appendChild(create('a', { className: 'btn btn-primary', href: '#', onClick: likeMovie }, 'Like'));
        }
    }
    const likeSpan = create('span', { className: 'enrolled-span' }, likes + 'like' + (likes === 1 ? '' : 's'));
    conrtrols.appendChild(likeSpan);

    const element = document.createElement('div');
    element.className = 'container';
    element.appendChild(create('div', { className: 'row bg-light text-dark' },
        create('h1', {}, `Movie title: ${movie.title}`),
        create('div', { className: 'col-md-8' },
            create('img', { className: 'img-thumbnail', src: movie.img })),
        conrtrols
    ));
